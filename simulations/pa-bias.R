...
# make another column with all of the true ps in it...
tp.col<-c()
for(i in c(1:4,17:20,33:36)){
   tp.col<-c(tp.col,rep(true.p$t[i],1000))
}

bb<-cbind(baf,tp.col)

bias<-c()
for(i in seq(1,nrow(bb),by=200)){
   bias<-rbind(bias,cbind(bb$n[i],bb$id[i],bb$model[i],
                          round(mean(bb$pa[i:(i+199)]-tp.col[i])/tp.col[i],3)))
}

for(ss in unique(bias[,1])){

   cat(ss)
   #"No covariates","Point transect","3-point","Covariate"
   for(mod in unique(bias[,3])){
      for(parid in unique(bias[,2])){
         ind<-(bias[,1]==ss) & (bias[,2]==parid) & (bias[,3]==mod)
         cat(" &",bias[which(ind),4])
      }
   } 
   cat("\\\\\n")
}
