...
plot.ds.mixture<-function(x,style="",main="",breaks="Sturges",ylim=NULL,xlim=NULL,pdf=FALSE,plot.formula=NULL,hide.hist=FALSE,nomf=FALSE,x.axis=NULL,xlab="Distance",ylab=NULL, this.id, ...){
   fit.object<-x
...
#            plot.seq<-c(plot.seq,length(this.col.levels))
  ...
  # plot the id
  points(x=1,y=0.85*max(a$density,p.at.zero),cex=3.75)
  text(x=1,y=0.85*max(a$density,p.at.zero),label=this.id)
