...
pdf(file="phidia.pdf",height=4.8,width=5)

par(mgp=c(1,1,0),mar=c(3,3,1,1),las=1)
...
text(x=0.1,y=Falpha[1]/2,lab=expression(phi[1]))
...
text(x=0.2,y=(Falpha[1]+Falpha[2])/2,lab=expression(phi[2]))
...
text(x=0.3,y=(Falpha[2]+Falpha[3])/2,lab=expression(phi[3]))
...
text(x=0.4,y=(Falpha[3]+Falpha[4])/2,lab=expression(phi[4]))
