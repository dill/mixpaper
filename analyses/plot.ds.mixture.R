...
plot.ds.mixture<-function(x,style="",main="",breaks="Sturges",ylim=NULL,xlim=NULL,pdf=FALSE,plot.formula=NULL,hide.hist=FALSE,nomf=FALSE,x.axis=NULL,xlab="Distance",ylab=NULL, pseq=NULL, lty=1, lcol=NULL...){
   fit.object<-x
   ...
   if((as.character(plot.formula) != "none") & !(pt&pdf)){
     ...
     for(curr.term in attr(terms.formula(plot.formula),"term.labels")){
        ...
        if(grepl("^as.factor",curr.term)){
          ...
          this.col.levels<-colnames(z[[1]])[grepl(curr.term,
                                                  colnames(z[[1]]),fixed=TRUE)]
        }else{
          # continuous covariates
          if(pseq==1){
            # estimate the 0.25, 0.5 and 0.75 quantiles
            this.col.levels<-quantile(z[[1]][,curr.term],c(0.25,0.5,0.75),
                                      names=FALSE)
          }else{
            this.col.levels<-unique(z[[1]][,curr.term])
          }
        }
...
#            plot.seq<-c(plot.seq,length(this.col.levels))
      ...
      k<-1
      pseq <- ifelse(is.null(pseq),1:length(plot.seq),pseq)
      if(pseq==3){
        k <- 9
      }else if(pseq==2){
        k <- 4
