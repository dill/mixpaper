...
\end{equation*}
where $\sum_{j=1}^J \phi_j = 1$. The (radial or perpendicular) distance is denoted $y$, the $\bm{\theta}_j$s are vectors of parameters for function $g_j$, $\bm{\theta}$ is a vector containing of all of the $\bm{\theta}_j$s, $\bm{\phi}$ is a vector of all of the $\phi_j$s. $\mathbf{z}$ is an a $K$-vector of the associated covariates.
...
Variances of both $N$ and $P_a$ were found for both non-covariate models and covariate models using standard methods (Borchers, Buckland and Zucchini, 2002, Appendix C; Marques and Buckland, 2004, Section 3.3.3; Innes et al., 1996).  [[ TKTKTK Consider putting into Web Appendix A and saying so here, or putting into a new Web Appendix C.]]
...
Web Appendices referenced in Section \ref{s:likelihood} [[TKTKTK add variance stuff if necessary]] are available with this paper at the Biometrics website on Wiley Online Library.
