...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{figs/lt-sim-detfcts.eps}\\
\includegraphics[width=\textwidth]{figs/pt-sim-detfcts.eps}
\caption{Plots of the models used in the simulation. Top row: detection functions for the line transect simulations with no covariates. Second row: pdfs for the point transect simulations with no covariates, the detection functions are as in the top row.}
\label{sim-detfcts}
\end{figure}
...
\begin{table}
\caption{Comparison of the results for the Williams and Thomas (2007) data. W\&T indicates the results reported in Williams and Thomas (2007), other results are from mixture models where the number of mixture components was selected by AIC. [[TKTKTK \textbf{LEN}: can you find the AICs for the other W\&T models?]]}
\centering
...
Humpback & hn+cos(2) (W\&T) & 2000 & 1033.06 & 0.386 & 12.64 & 0.6716 \\
 & 2-point & 2000 & 1035.94 & 0.381 & 15.6 & 0.649 \\
\end{tabular}
\label{williams-table}
\end{table}

[[\textbf{LEN}: need to sort out what to say about the Dall's porpoise... The model in the paper wasn't best? responsive movement, p. 25 Williams and Thomas (2007)]]
...
[[TKTKTK say something about Dall's, esp. spike.]]

\begin{figure}
\centering
\includegraphics[width=\textwidth]{analyses/williamsplots.eps}
\caption{Plots of the detection functions fit to the Williams and Thomas (2007) data. In each case the best model by AIC was a 2-point mixture.}
...
[[\textbf{LEN}: can I get a copy of the paper?]]

\begin{table}
\caption{ANTS RESULTS. Truncation at 25m. Mixtures selected as best by AIC. [[\textbf{LEN}: can you find the KS $p$-values? Can you also run the missing models? These seem very different, did you use 25m truncation when you ran these? Bold indicates lowest AIC.]]}
\centering
...
Hn 2-pt & habitat & 751.27 & 0.188 & 0.3 & 0.97 \\ 
Hn 2-pt & species & 756.59 & 0.184 & 0.1 & 0.94 \\  
Hn 2-pt & nest size & 741.64 & 0.214 & 10.9 & 0.76 \\   
Hn 2-pt & species+habitat & 749.0516 & 0.0272 & 38.7 & 0.94\\ 
Hn 2-pt & nest.size+habitat & \textbf{737.27} & 0.179 & 13.6 & 0.72\\
Hn 2-pt & nest.size+species & 741.92 & 0.210 & 10.9 & 0.77\\
Hn 2-pt & nest.size+species+habitat & 739.77 & 0.191 & 12.9 & 0.68 \\
\end{tabular}
\label{ants-table}
\end{table}
...
Pike \textit{et al} (2003) analyse data on long-finned pilot whales (\textit{Globicephala melas}). There were 84 pods sighted as part of the NASS-2001 survey (of which the pilot whale was not a target). The Beaufort sea state was recorded as a covariate during the survey and enter the model as either a 4 level factor (\texttt{BSS}), a 3 level factor (\texttt{BSS3}) or a 2 levels factor (\texttt{BSS2}).

A model was fit with each of these covariates, as well as a model with no covariates. A summary of the results is given in table \ref{pilot-table} and the detection function for the best model is shown in figure [[TKTKTK whale detfct plot]].
\begin{table}
\caption{PILOT WHALE RESULTS. (P) indicates results from Pike et al. (2003). [[TKTKTK results from Distance.]] Truncation at 3000m. Bold indicates the model with the lowest AIC.}
\centering
\begin{tabular}{c c c c c c}
Model & Covariate & AIC & $\hat{P_a}$ & $\% CV \hat{P_a}$ & K-S $p$\\
\hline
\hline
2-point & None & 1298.42 & 0.295 & 23.6 & 0.9471 \\
2-point & \texttt{BSS} & 1286.6 & 0.209 & 8.54 & 0.8196 \\
2-point & \texttt{BSS2} & \textbf{1284.99} & 0.211 & 8.5 & 0.9533\\
2-point & \texttt{BSS3} & 1296.27 & 0.2698 & 4.88 & 0.9864 \\
\end{tabular}
\label{pilot-table}
\end{table}

The best model by AIC score was  the \texttt{BSS2} model. A plot of the average detection function... [[TKTKTK plot]]
...
\begin{itemize}
	...
	\item \texttt{has} - hours after sunrise (factor).
\end{itemize}
Given that this is a large data set, it may well support the incorporation of 2 or more of the covariates above.

[[Big results table here comparing the results from the relevant papers and what we got?? A la Morgan \& Ridout?]]
...
\textbf{LEN}: do we need a reference for rejection sampling?
...
\bibitem{ } Beavers, S. C., and Ramsey, F. L. (1998). Detectability analysis in transect surveys. \textit{Journal of Wildlife Management} \textbf{62}(3), pp. 948Ð957.
...
\bibitem{ } Morgan, B. J. T. and M. Ridout (2008). A new mixture model for capture heterogeneity. \textit{Journal of the Royal Statistical Society: Series C} \textbf{57}(4), 433Ð446. 

\bibitem{ } Pledger, S. (2000). Unified maximum likelihood estimates for closed capture-recapture models using mixtures. \textit{Biometrics} \textbf{56}(2), 434Ð442. 

\bibitem{ } Pledger, S. (2005). The performance of mixture models in heterogeneous closed population capture-recapture. \textit{Biometrics} \textbf{61}(3), 868Ð873.
