...
COMMENTS:
	nocov - fine, converge to true p AND to true model
	pt - fine but in 2,4 they look more like 1 point models, see this a bit in nocov but more so here
	covar - good! selecting the covariate model even when it could just select the 3 point, but some evidence that this was the case at lower sample sizes (TKTKTK give results for time in each model here?) for 120 samples looks right median but with large IQR
	3pt - first sim would have been fine as a 2 point model -- see fig 2. Second sim more like a real 3-point model but takes up to a very large sample size to see it. probably the very large sigma component just gets confounded with one of the others.
...
Hn 2-pt & \texttt{habitat} & 751.27 & 0.150 & 20.0 & 0.97 \\ 
...
%\end{align*}
%So then using the sandwich estimator Borchers et al. (2002):
%\begin{align*}
